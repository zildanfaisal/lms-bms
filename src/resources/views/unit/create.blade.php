...
@section('header')
    <h1 class="text-xl font-semibold text-gray-800">Tambah Unit</h1>
@endsection
        ...
        <form action="{{ route('unit.store') }}" method="POST">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    @php $selectedDivisi = old('divisi_id', request('divisi_id')); @endphp
                    <select name="divisi_id" class="mt-1 block w-full rounded border-gray-200 tom-select" required>
                        <option value="">-- Pilih Divisi --</option>
                        @foreach($divisis as $div)
                            <option value="{{ $div->id }}" {{ $selectedDivisi == $div->id ? 'selected' : '' }}>{{ $div->nama_divisi }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="nama_unit" value="{{ old('nama_unit') }}" class="mt-1 block w-full rounded border-gray-200" required>
                </div>
